...
export default function SignalCard({ signal }: { signal: Signal }) {
  return (
    <Link href={`/signals/${signal.id}`} className="block group">
      <div className="bg-white rounded-lg border border-[#E0DDD6] p-4 hover:border-[#8A6218] hover:shadow-sm transition-all">
        <div className="flex items-start justify-between gap-3 mb-2">
          <p className="text-sm font-semibold text-[#1E3651] group-hover:text-[#8A6218] transition-colors leading-snug">
            {signal.title}
          </p>
          <PriorityBadge priority={signal.priorityLabel} />
        </div>
        <p className="text-xs text-[#666] line-clamp-2 mb-3">{signal.summary}</p>
        <div className="flex items-center gap-3 text-[10px] text-[#6E6E6E] font-medium">
