    ...
    <>
      ...
      <div id="ca-modal-bm" role="dialog" aria-modal="true" style={{display:'none',position:'fixed',inset:0,zIndex:10000,background:'rgba(0,0,0,.8)',alignItems:'center',justifyContent:'center'}} onClick={(e) => { if (e.target === e.currentTarget) { const el = document.getElementById('ca-modal-bm'); if (el) el.style.display = 'none'; } }}>
        <div style={{background:'#fff',maxWidth:420,width:'90%',padding:'2rem',borderRadius:4}}>
          ...
          <input id="ca-name-bm" aria-label="Your name" placeholder="Name" style={{width:'100%',padding:'.6rem',marginBottom:'.75rem',border:'1px solid #ccc',boxSizing:'border-box',fontFamily:'sans-serif'}}/>
          <input id="ca-email-bm" aria-label="Your email address (required)" type="email" placeholder="Email (required)" style={{width:'100%',padding:'.6rem',marginBottom:'.75rem',border:'1px solid #ccc',boxSizing:'border-box',fontFamily:'sans-serif'}}/>
          <textarea id="ca-msg-bm" aria-label="Your message" rows={3} placeholder="Message" style={{width:'100%',padding:'.6rem',marginBottom:'.75rem',border:'1px solid #ccc',boxSizing:'border-box',fontFamily:'sans-serif',resize:'vertical'}}></textarea>
